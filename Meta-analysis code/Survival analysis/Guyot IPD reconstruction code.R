guyot.recon.KM <- function(KMdata,pub.risk,tot.events=NA){
  

#Algorithm to create a raw dataset from readings from a Kaplan-Meier curve
#require("MASS")
#require("splines")
require("survival")
...
# output data
KM.out.data <- data.frame(t.S,n.hat=n.hat[1:n.t],d,cen)
...
IPD <- data.frame(t.IPD,event.IPD,arm)

out <- list(KM.out.data=KM.out.data, IPD=IPD)
return(out)
